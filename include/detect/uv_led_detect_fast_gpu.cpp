...
bool uvdar::UVDARLedDetectFASTGPU::processImage(const cv::Mat i_image, std::vector<cv::Point2i>& detected_points, std::vector<cv::Point2i>& sun_points, int mask_id) {
  ...
  std::cerr << "[UVDARDetectorFASTGPU]: Starting processing..." << std::endl;
  detected_points = std::vector<cv::Point2i>();
  sun_points = std::vector<cv::Point2i>();
  image_curr_     = i_image;

  std::cerr << "[UVDARDetectorFASTGPU]: Checking if initialized..." << std::endl;
  if (!initialized_) {
  ...
  std::cerr << "[UVDARDetectorFASTGPU]: Checking mask index..." << std::endl;
  ...
  std::cerr << "[UVDARDetectorFASTGPU]: Mask index is ok." << std::endl;

  std::cerr << "[UVDARDetectorFASTGPU]: Checking mask size..." << std::endl;
  if (mask_id >= 0){
  ...
  std::cerr << "[UVDARDetectorFASTGPU]: Checking mask size is ok." << std::endl;

  if (_gui_) {
    std::cerr << "[UVDARDetectorFASTGPU]: Copying to viewer image..." << std::endl;
  ...
  uint32_t markers_cnt_val;
    ...
    std::cerr << "[UVDARDetectorFASTGPU]: Resetting atomic counter buffer objects..." << std::endl;
  ...
  compute_lib_acbo_write_uint_val(&compute_prog, &sun_pts_count_acbo, 0);
    ...
    std::cerr << "[UVDARDetectorFASTGPU]: Writing data to GPU memory..." << std::endl;
  ...
  std::cerr << "[UVDARDetectorFASTGPU]: Dispatching shader..." << std::endl;
  // dispatch compute shader
  compute_lib_program_dispatch(&compute_prog, image_size.width / local_size_x, image_size.height / local_size_y, 1);

  std::cerr << "[UVDARDetectorFASTGPU]: Retrieving detected markers..." << std::endl;
  ...
  std::cerr << "[UVDARDetectorFASTGPU]: Retrieving detected sun points..." << std::endl;
  ...
  std::cerr << "[UVDARDetectorFASTGPU]: Finding centroids of detected markers..." << std::endl;
  // find centroids of concentrated detected markers
  cpuFindMarkerCentroids(markers, markers_cnt_val, 5, detected_points);

  std::cerr << "[UVDARDetectorFASTGPU]: Converting sun points..." << std::endl;
  ...
  std::cerr << "[UVDARDetectorFASTGPU]: Done." << std::endl;
  return true;
}
