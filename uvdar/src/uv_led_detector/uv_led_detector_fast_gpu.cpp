#include <uvdar/uv_led_detector/uv_led_detect_fast_gpu.h>
...
struct FastGpuResources {
  ...
  void rebuild_params() {
    params = {image_in, image_mask, detected_markers, marker_counter, detected_suns};
  }
};
//}
...
UvdarLedDetectFastGpu::~UvdarLedDetectFastGpu() = default;
...
void UvdarLedDetectFastGpu::initGpuComputing_(const int width, const int height) {
  ...
  gpu_resources_->eval_fast_ring_alg =
      gpu.algorithm(gpu_resources_->params, loadPrecompiledShader_("uvdar", "eval_fast_ring.spv"),
