...
void AmiTracker::enforceMaxBufferLength_(std::vector<PointState>& unmatched_points) {
  if (active_tseries_buffer_.size() > cfg_.max_buffer_length) {
    ...
    active_tseries_buffer_.erase(active_tseries_buffer_.begin() + cfg_.max_buffer_length, active_tseries_buffer_.end());
  }
}
//}
...
/* cleanPotentialBuffer_ //{ */
/// @brief Remove sequences whose signal has been missing for too long
void AmiTracker::cleanPotentialBuffer_() {
  ...
  while (it != active_tseries_buffer_.end()) {
    ...
    int zero_counter = countNumConsequtiveZerosInTseries_(tseries, delete_criterion);
...
/* countNumConsequtiveZerosInTseries_ //{ */
int AmiTracker::countNumConsequtiveZerosInTseries_(SeqPtr& tseries, const int max_num_zeros) {
  int zero_counter{0};
