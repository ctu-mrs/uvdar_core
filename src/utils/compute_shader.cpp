    ...
    while (error_queue->size() > 0) {
        ...
        if (out != nullptr && error != nullptr) {
            std::fprintf(
                ...
                error->err_id,
                uvdar_core::utils::GLES::get_define_name(error->type),
                error->type,
                uvdar_core::utils::GLES::get_define_name(error->severity),
                error->severity,
                error->message.c_str());
        }
        count++;
...
GLuint Image2D::init(GLenum framebuffer_attachment)
{
    ...
    const std::size_t type_size = uvdar_core::utils::GLES::get_type_size(type);
...
std::string Image2D::glsl_layout()
{
    ...
    const int result = asprintf(
        ...
        uvdar_core::utils::GLES::get_glsl_image2d_format_qualifier(compatibility_format),
        resource.value,
        uvdar_core::utils::GLES::get_glsl_image2d_access(access),
        uvdar_core::utils::GLES::get_glsl_image2d_type(compatibility_format),
        resource.name.c_str());
...
GLuint ACBO::write(const void* data, GLint len)
{
    ...
    glBufferData(GL_ATOMIC_COUNTER_BUFFER, uvdar_core::utils::GLES::get_type_size(type) * len, data, usage);
...
{
    glBindBuffer(GL_ATOMIC_COUNTER_BUFFER, handle);
    const GLsizei size = uvdar_core::utils::GLES::get_type_size(type) * len;
...
std::string SSBO::glsl_layout()
{
    ...
    const int result = asprintf(
        ...
        resource.value,
        resource.name.c_str(),
        uvdar_core::utils::GLES::get_glsl_data_type(type),
        resource.name.c_str());
    std::string out = (result < 0 || str == nullptr) ? std::string { } : std::string(str);
    if (str != nullptr) {
        std::free(str);
...
GLuint SSBO::write(const void* data, GLint len)
{
    ...
    glBufferData(GL_SHADER_STORAGE_BUFFER, uvdar_core::utils::GLES::get_type_size(type) * len, data, usage);
...
{
    glBindBuffer(GL_SHADER_STORAGE_BUFFER, handle);
    const GLint size = uvdar_core::utils::GLES::get_type_size(type) * len;
